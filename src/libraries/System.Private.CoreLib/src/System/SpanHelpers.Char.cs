...
namespace System
{
    internal static partial class SpanHelpers // .Char
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public static unsafe int IndexOfAny2(ref char searchStart, char value0, char value1, int length)
        {
            Debug.Assert(length >= 0);
        ...
        SequentialScan:
            ...
            while (lengthToExamine >= 4)
            {
                ref char current = ref Unsafe.Add(ref searchStart, offset);
            ...
            while (lengthToExamine > 0)
            {
                lookUp = Unsafe.Add(ref searchStart, offset);
                if (value0 == lookUp || value1 == lookUp)
                    goto Found;
